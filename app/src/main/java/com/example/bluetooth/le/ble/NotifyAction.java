...
import com.example.bluetooth.le.HuamiService;
...
public class NotifyAction extends BtLEAction {
    ...
    public boolean run(BluetoothGatt gatt) {
        ...
        if (result) {
            BluetoothGattDescriptor notifyDescriptor = getCharacteristic().getDescriptor(HuamiService.UUID_DESCRIPTOR_GATT_CLIENT_CHARACTERISTIC_CONFIGURATION);
            if (notifyDescriptor != null) {
                int properties = getCharacteristic().getProperties();
                if ((properties & BluetoothGattCharacteristic.PROPERTY_NOTIFY) > 0) {
                    Log.d("NotifyAction","use NOTIFICATION");
                    ...
                    Log.d("NotifyAction","use INDICATION");
                ...
                Log.w("NotifyAction","Descriptor CLIENT_CHARACTERISTIC_CONFIGURATION for characteristic " + getCharacteristic().getUuid() + " is null");
                hasWrittenDescriptor = false;
            }
        } else {
            hasWrittenDescriptor = false;
            Log.e("NotifyAction","Unable to enable notification for " + getCharacteristic().getUuid());
        }
        return result;
    }
