...
import com.example.bluetooth.le.GattServerCallback;
import com.example.bluetooth.le.HADevice;
import com.example.bluetooth.le.HealthApplication;
import com.example.bluetooth.le.Transaction;
...
public final class BtLEQueue {
    ...
    private final HADevice mDevice;
    ...
    private Thread dispatchThread = new Thread("Gadgetbridge GATT Dispatcher") {
        ...
        @Override
        public void run() {
            Log.d("BtLEQueue","Queue Dispatch Thread started.");
            ...
            while (!mDisposed && !mCrashed) {
                try {
                    ...
                    if (!isConnected()) {
                        Log.d("BtLEQueue","not connected, waiting for connection...");
...
*/
                    if(qTransaction instanceof Transaction) {
                        ...
                        for (BtLEAction action : transaction.getActions()) {
                            if (mAbortTransaction) { // got disconnected
                                Log.i("BtLEQueue","Aborting running transaction");
                                break;
                            }
                            ...
                            if (action.run(mBluetoothGatt)) {
                                ...
                                Log.e("BtLEQueue","Action returned false: " + action);
                                break; // abort the transaction
                            }
                        }
                    }
                    ...
                    mConnectionLatch = null;
                    Log.d("BtLEQueue","Thread interrupted");
                } catch (Throwable ex) {
                    Log.e("BtLEQueue","Queue Dispatch Thread died: " + ex.getMessage(), ex);
                    mCrashed = true;
                    mConnectionLatch = null;
                } finally {
                    mWaitForActionResultLatch = null;
            ...
            Log.i("BtLEQueue","Queue Dispatch Thread terminated.");
        }
    };

    public BtLEQueue(BluetoothAdapter bluetoothAdapter, HADevice device, BluetoothGattCallback externalGattCallback, BluetoothGattCallback externalGattServerCallback, Context context) {
        mBluetoothAdapter = bluetoothAdapter;
        this.mDevice = device;
        this.mWaitForServerActionResultLatch = mWaitForServerActionResultLatch;
        internalGattCallback = new InternalGattCallback((GattCallback) externalGattCallback);
        internalGattServerCallback = new InternalGattServerCallback((GattServerCallback) externalGattServerCallback);
        mContext = context;
     ...
     * specific initialization. This should be done in the specific
     * class.
     *
    ...
    public boolean connect() {
        if (isConnected()) {
            Log.w("BtLEQueue","Ignoring connect() because already connected.");
            return false;
        }
        synchronized (mGattMonitor) {
            if (mBluetoothGatt != null) {
                ...
                // so create a new one.
                Log.i("BtLEQueue","connect() requested -- disconnecting previous connection: " + mDevice.getDeviceName());
                disconnect();
            }
        }
        Log.i("BtLEQueue","Attempting to connect to " + mDevice.getDeviceName());
        mBluetoothAdapter.cancelDiscovery();
        BluetoothDevice remoteDevice = mBluetoothAdapter.getRemoteDevice(mDevice.getDeviceAddress());
            BluetoothManager bluetoothManager = (BluetoothManager) mContext.getSystemService(Context.BLUETOOTH_SERVICE);
            if (bluetoothManager == null) {
                Log.e("BtLEQuque","Error getting bluetoothManager");
                return false;
            }
            mBluetoothGattServer = bluetoothManager.openGattServer(mContext, internalGattServerCallback);
            if (mBluetoothGattServer == null) {
                Log.e("BtLEQueue","Error opening Gatt Server");
                return false;
            }
        ...
        synchronized (mGattMonitor) {
            ...
            if (HealthApplication.isRunningMarshmallowOrLater()) {
        ...
        if (result) {
            setDeviceConnectionState(HADevice.State.CONNECTING);
        }
        return result;
    }

    private void setDeviceConnectionState(HADevice.State newState) {
        Log.d("BLE","new device connection state: " + newState);
    ...
    public void disconnect() {
        synchronized (mGattMonitor) {
            ...
            if (gatt != null) {
                mBluetoothGatt = null;
                Log.d("BLE","Disconnecting BtLEQueue from GATT device");
                gatt.disconnect();
                gatt.close();
                setDeviceConnectionState(HADevice.State.NOT_CONNECTED);
            }
    ...
    private void handleDisconnected(int status) {
        Log.d("BLE","handleDisconnected: " + status);
        internalGattCallback.reset();
        mTransactions.clear();
    ...
    private boolean maybeReconnect() {
        if (mAutoReconnect && mBluetoothGatt != null) {
            Log.d("BLE","Enabling automatic ble reconnect...");
            boolean result = mBluetoothGatt.connect();
            if (result) {
                setDeviceConnectionState(HADevice.State.WAITING_FOR_RECONNECT);
            }
            return result;
        }
        return false;
...
//        }
    ...
    public void add(Transaction transaction) {
        Log.d("BLE","about to add: " + transaction);
...
*/
    ...
    public void insert(Transaction transaction) {
        Log.d("BtLEQueue","about to insert: " + transaction);
        if (!transaction.isEmpty()) {
    ...
    public List<BluetoothGattService> getSupportedGattServices() {
        if (mBluetoothGatt == null) {
            Log.w("BLE","BluetoothGatt is null => no services available.");
    ...
    private boolean checkCorrectGattInstance(BluetoothGatt gatt, String where) {
        if (gatt != mBluetoothGatt && mBluetoothGatt != null) {
            Log.i("BLE","Ignoring event from wrong BluetoothGatt instance: " + where + "; " + gatt);
            return false;
        }
        return true;
    }
    ...
    private boolean checkCorrectBluetoothDevice(BluetoothDevice device) {
        ...
        if(!device.getAddress().equals(mDevice.getDeviceAddress())) { // != clientDevice && clientDevice != null) {
            Log.d("BLE","Ignoring request from wrong Bluetooth device: " + device.getAddress());
            return false;
        }
        return true;
    }
    ...
    private final class InternalGattCallback extends BluetoothGattCallback {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            Log.d("BLE","connection state change, newState: " + newState + getStatusString(status));
            ...
            if (status != BluetoothGatt.GATT_SUCCESS) {
                Log.w("BLE","connection state event with error status " + status);
            }

            switch (newState) {
                case BluetoothProfile.STATE_CONNECTED:
                    Log.i("BLE","Connected to GATT server.");
                    setDeviceConnectionState(HADevice.State.CONNECTED);
                    // Attempts to discover services after successful connection.
                    List<BluetoothGattService> cachedServices = gatt.getServices();
                    if (cachedServices != null && cachedServices.size() > 0) {
                        Log.d("BLE","Using cached services, skipping discovery");
                        onServicesDiscovered(gatt, BluetoothGatt.GATT_SUCCESS);
                    } else {
                        Log.d("BLE","Attempting to start service discovery");
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            Log.d("BLE","characteristic write: " + characteristic.getUuid() + getStatusString(status));
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
                                         ...
                                         int status) {
            Log.d("BLE","characteristic read: " + characteristic.getUuid() + getStatusString(status));
            ...
            if (getCallbackToUse() != null) {
                try {
                    ...
                    Log.d("BLE","onCharacteristicRead: " + ex.getMessage(), ex);
                }
            }
            checkWaitingCharacteristic(characteristic, status);
        }
        ...
        public void onDescriptorRead(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            Log.d("BLE","descriptor read: " + descriptor.getUuid() + getStatusString(status));
            ...
            if (getCallbackToUse() != null) {
                try {
                    ...
                    Log.d("BLE","onDescriptorRead: " + ex.getMessage(), ex);
                }
            }
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            Log.d("BLE","descriptor write: " + descriptor.getUuid() + getStatusString(status));
            ...
            if (getCallbackToUse() != null) {
                try {
                    ...
                    Log.d("BLE","onDescriptorWrite: " + ex.getMessage(), ex);
                }
            }
            checkWaitingCharacteristic(descriptor.getCharacteristic(), status);
        }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
                ...
                Log.d("BtLEQueue","characteristic changed: " + characteristic.getUuid() + " value: " + Arrays.toString(characteristic.getValue()));
            ...
            if (getCallbackToUse() != null) {
                try {
                    ...
                    Log.e("BtLEQueue","onCharaceristicChanged: " + ex.getMessage(), ex);
                }
            } else {
                Log.i("BtLEQueue","No gattcallback registered, ignoring characteristic change");
            }
        }

        @Override
        public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
            Log.d("BtLEQueue","remote rssi: " + rssi + getStatusString(status));
            ...
            if (getCallbackToUse() != null) {
                try {
                    ...
                    Log.e("BtLEQueue","onReadRemoteRssi: " + ex.getMessage(), ex);
                }
            }
        }
        ...
        private void checkWaitingCharacteristic(BluetoothGattCharacteristic characteristic, int status) {
            if (status != BluetoothGatt.GATT_SUCCESS) {
                if (characteristic != null) {
                    Log.d("BtLEQueue","failed btle action, aborting transaction: " + characteristic.getUuid() + getStatusString(status));
                }
                mAbortTransaction = true;
            }
            if (characteristic != null && BtLEQueue.this.mWaitCharacteristic != null && characteristic.getUuid().equals(BtLEQueue.this.mWaitCharacteristic.getUuid())) {
                ...
                if (BtLEQueue.this.mWaitCharacteristic != null) {
                    Log.e("BtLEQueue","checkWaitingCharacteristic: mismatched characteristic received: " + ((characteristic != null && characteristic.getUuid() != null) ? characteristic.getUuid().toString() : "(null)"));
                }
            }
        }
        ...
        public void reset() {
            ...
            Log.d("BtLEQueue","internal gatt callback set to null");
    ...
    private final class InternalGattServerCallback extends BluetoothGattServerCallback {
        ...
        public void onConnectionStateChange(BluetoothDevice device, int status, int newState) {
            Log.d("BtLEQueue","gatt server connection state change, newState: " + newState + getStatusString(status));
            ...
            if (status != BluetoothGatt.GATT_SUCCESS) {
                Log.w("BtLEQueue","connection state event with error status " + status);
            }
        }
        ...
        public void onCharacteristicReadRequest(BluetoothDevice device, int requestId, int offset, BluetoothGattCharacteristic characteristic) {
            ...
            Log.d("BtLEQueue","characterstic read request: " + device.getAddress() + " characteristic: " + characteristic.getUuid());
            if (getCallbackToUse() != null) {
        ...
        public void onCharacteristicWriteRequest(BluetoothDevice device, int requestId, BluetoothGattCharacteristic characteristic, boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
            ...
            Log.d("BtLEQueue","characteristic write request: " + device.getAddress() + " characteristic: " + characteristic.getUuid());
            if (getCallbackToUse() != null) {
        ...
        public void onDescriptorReadRequest(BluetoothDevice device, int requestId, int offset, BluetoothGattDescriptor descriptor) {
            ...
            Log.d("BtLEQueue","onDescriptorReadRequest: " + device.getAddress());
            if(getCallbackToUse() != null) {
        ...
        public void onDescriptorWriteRequest(BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor, boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
            ...
            Log.d("BtLEQueue","onDescriptorWriteRequest: " + device.getAddress());
            if(getCallbackToUse() != null) {
